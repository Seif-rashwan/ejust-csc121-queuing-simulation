...
void ServerListType::setServerBusy(int server_id, const CustomerType& cCustomer) {
    if (server_id >= 0 && server_id < numOfServers) {
        servers[server_id].setBusy();
        servers[server_id].setCurrentCustomer(cCustomer);
        servers[server_id].setTransactionTimeFromCustomer();
        //
        std::cout << "  [Server " << (server_id + 1)
                  << "] now serving Customer " << cCustomer.getCustomerNumber()
                  << " (will take " << cCustomer.getTransactionTime()
                  << " ticks)" << std::endl;
    }
}
