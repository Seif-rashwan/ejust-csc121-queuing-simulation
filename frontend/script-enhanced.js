...
// #endregion
// ─────────────────────────────────────────────────────────
// #region - CircularArrayRingRenderer
// Renders a visual circular array representation of the queue
// ─────────────────────────────────────────────────────────
const RING = {
  MAX_SLOTS: 100,
  canvas: null,
  ...
  draw(queueSize, maxSlots, _, servers) {
    ...
    const max_slots = maxSlots;

    // Scale dot size down for large queues so they still fit on the ring
    let r_size;
    if (max_slots <= 100) {
      r_size = 10;
    } else if (max_slots <= 200) {
      r_size = 7;
    } else if (max_slots <= 500) {
      r_size = 5;
    } else {
      r_size = 3;
    }
...
async function fetchState() {
  const res = await fetch(BACKEND_CONFIG.baseUrl + BACKEND_CONFIG.endpoints.state);
  if (!res.ok) throw new Error("State fetch failed");
  return res.json();
}

async function sendConfig(cfg) {
  const r = await fetch(BACKEND_CONFIG.baseUrl + BACKEND_CONFIG.endpoints.config, {
    method: "POST",
...
async function sendPause() {
  await fetch(BACKEND_CONFIG.baseUrl + BACKEND_CONFIG.endpoints.pause, { method: "POST" });
}

async function sendReset() {
  await fetch(BACKEND_CONFIG.baseUrl + BACKEND_CONFIG.endpoints.reset, { method: "POST" });
}

async function sendStart() {
  await fetch(BACKEND_CONFIG.baseUrl + BACKEND_CONFIG.endpoints.start, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      simulationTime: 1000,
...
// #endregion
// ─────────────────────────────────────────────────────────
// #region - Rendering & Visualization
// ─────────────────────────────────────────────────────────
const SERVER_COLORS = [
  "#6c8aff",
  "#f7b731",
